...
export async function generateMetadata({
  ...
  const data = await getPersonById(movieId);
...
export default async function PeoplePage({
  ...
  params: { personId: string; slug: string };
}) {
    ...
    <section className="md:p-4 dark:text-white">
      <section className="flex flex-col md:flex-row gap-3">
        ...
        <section className="flex flex-col px-3 md:px-0 gap-3 text-sm">
          ...
          <p>{data?.biography}</p>
          <p>
            Birthday: <span className="font-bold">{data?.birthday}</span>
          </p>
          <p>
            Place of birth:{" "}
            <span className="font-bold">{data?.place_of_birth}</span>
          </p>
          ...
          <p>
            Also known as:{" "}
            <span className="font-bold">{data?.also_known_as.join(", ")}</span>
          </p>
        </section>
      </section>
